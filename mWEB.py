...
# === Página de Importação ===
    ...
    with col3:
        if tipo_teste == "Registro inercial livre":
            st.markdown( """ <h1 style='text-align: center; color: #1E90FF;'> 🧍🏽‍♀️Registro livre </h1> """, unsafe_allow_html=True )
            html = dedent(""" <div style="text-align: justify; font-size: 1.1rem; line-height: 1.6; color: #333333; max-width: 900px; margin: auto; background-color: rgba(255,200,255,0.6); padding: 20px; border-radius: 8px;">
    ...
    if "dados" in st.session_state and "tipo_teste" in st.session_state:
        ...
        if tipo_teste == "Registro inercial livre":
            ...
            with col1:
                ...
                filtrado = st.checkbox('Filtrar abaixo de?',False)
            with col2:
                if detrend_command:
                    x_new = detrend(x)
                    y_new = detrend(y)
                ...
                if filtrado:
                    ...
                    cutoff = st.number_input("Frequência de corte (Hz)",min_value=0.1,max_value=float(fs_mean / 2),value=10.0)
                    b, a = butter_lowpass(cutoff, fs, 2)
                ...
                if separados:
                    ...
                    st.pyplot(fig)
                    fig, ax = plt.subplots(figsize=(10, 4))
                    ax.plot(t, y_new,'-g', label='y')                    
                    ax.set_xlabel('Tempo (ms)')
                    ax.set_ylabel('Amplitude')
                    ax.legend()
                    st.pyplot(fig)
                    fig, ax = plt.subplots(figsize=(10, 4))
                    ax.plot(t, z_new,'-k', label='z')
                    ax.set_xlabel('Tempo (ms)')
                    ax.set_ylabel('Amplitude')
                    ax.legend()
                    st.pyplot(fig)
